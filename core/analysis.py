    ...
    try:
        ...
        if icc_type == 'ICC(3,1)':
            ...
            if denominator <= 0:  # Avoid division by zero or negative denominator
                st.warning(
                    f"ICC denominator non-positive ({denominator:.2f}). Cannot calculate reliable ICC(3,1). May indicate no variance between subjects or high error variance.")
...
def interpret_icc(icc_value):
    ...
    if icc_value < 0.5:
        category, color = "Poor reliability", "#dc3545"  # Red
    elif icc_value < 0.75:
        category, color = "Moderate reliability", "#ffc107"  # Yellow/Orange
    elif icc_value < 0.9:
        category, color = "Good reliability", "#198754"  # Green
    else:
        category, color = "Excellent reliability", "#0d6efd"  # Blue
...
def create_icc_visualization(manual_ratings: pd.Series, llm_ratings: pd.Series, icc_value):
    ...
    try:
        ...
        # --- Scatter plot ---
        ax1 = axes[0]
        sns.regplot(x=manual_valid, y=llm_valid, ax=ax1, scatter_kws={'alpha': 0.5},
        ...
        if not pd.isna(std_diff) and std_diff > 1e-6:  # Avoid drawing if std dev is tiny/zero
            ax2.axhline(limit_of_agreement_upper, color='grey', linestyle='--',
                        label=f"+1.96 SD ({limit_of_agreement_upper:.2f})")
            ax2.axhline(limit_of_agreement_lower, color='grey', linestyle='--',
                        label=f"-1.96 SD ({limit_of_agreement_lower:.2f})")
        else:
...
def perform_icc_computation(df: pd.DataFrame, response_column: str, manual_columns: list):
    ...
    try:
        ...
        for col in manual_columns:
            ...
            df_analysis[numeric_col_name] = try_convert_to_numeric(df_analysis[col],
                                                                   column_name=f"Manual Score ({col})")
        ...
        if valid_rows < original_rows:
            st.warning(
                f"Removed {original_rows - valid_rows} rows due to missing/non-numeric scores in required columns before calculating ICC.")

        if valid_rows < 2:
            st.error(
                f"Not enough valid subjects ({valid_rows}) with complete ratings across all selected columns to compute ICC (minimum 2 required).")
            return None, df_analysis, None  # Return df with attempted conversions
